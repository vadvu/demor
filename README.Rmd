...
4. Age-standardized *YLL* rate: $$ASYR_{x,t,c} = \sum_x^\omega{[YLL^r_{x,t,c}*W_x]}$$ where $W_x$ is the standard population weight at *x*. In other words, it's just direct standardization of $YLL^r_{x,t,c}$.    
...
Also in the `demor` there is `leecart()` function that provides users with basic *Lee-Carter model* for mortality forecasting:
