...
Note, one can calculate not only Gini *inequality* index, but also Drewnowski’s index of *equality* that is 
$D_0 = 1 - G_0$ proposed by @Drewnowski (so, for Russian 1995 females it is $1-0.13=0.87$) that also can serve as an indicator of the shape of mortality patterns.
...
Also one can do simple *decomposition* between 2 populations. Lets use Russia-2000 as *base population* and Russia-2010 as *compared population*. This function implements two almost identical discrete methods that were proposed almost simultaneously: first in @andreev_dec and the second in @arriaga_dec.  
...
The comparison between different methods for Lee-Carter is presented below.


```{r, echo=FALSE}
data = demor::rosbris_mortality_pop_1 %>% filter(territory == "t" & sex == "m" & code == 1100 & year <= 2019)
...
lc_plot <- leecart_forecast$ex0 %>% mutate(model = "Original\n1989-2019\n") %>% 
  rbind(x = ., y = leecart_forecast_2000$ex0 %>% mutate(model = "Original\n2000-2019\n")) %>% 
  rbind(x = ., y = leecart_forecast_2000_last$ex0 %>% mutate(model = "ax correction\n2000-2019\n")) %>% 
  rbind(x = ., y = leecart_forecast_last$ex0 %>% mutate(model = "ax correction\n1989-2019\n"))
...
lc_plot %>% 
  ...
  geom_point(aes(y = e0.obs, color = "Observed")) +
  geom_line(aes(y = e0.obs, color = "Observed")) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, color = model), alpha = 0.3, fill = "pink", linetype = "dashed")+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  facet_wrap(~model) +
  scale_color_manual(values = c("steelblue", "darkred", "black", "orange", "forestgreen"))+
...
les <- leslie(mx = mx, fx = fx, age.mx = 0:100, age.fx = 15:55, sex = "f", fin = F)
...
Nt <- ccm(Mx.f = const.mx, Fx = const.fx, age.mx = 0:100, age.fx = 15:55, N0.f = N0)
...
Also one can redesigned plot using [ggplot2](https://github.com/tidyverse/ggplot2) commands. Also one can plot additional population (`popm2` and `popf2`) pyramid as dashed line that can be helpfull for comparisons.
...
plot <- 
  ...
  popm2 = dbm[dbm$year==2000 & dbm$code==1100 & dbm$territory=="t" & dbm$sex=="m",]$N,
  popf2 = dbm[dbm$year==2000 & dbm$code==1100 & dbm$territory=="t" & dbm$sex=="f",]$N,
