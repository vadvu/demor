...
The data from @rosbris can be downloaded into `demor` in the long format (mortality/fertility by 1/5-year age groups from 1989 to 2022) using `get_rosbris()`. Earlier versions of `demor` bundled RosBris tables directly, but they were removed from the package at the request of the rights holder, so RosBris data are now available only via download. Downloaded archives are cached locally, so repeated calls reuse them unless `refresh = TRUE`.\
...
The data from @rosbris can also be downloaded as *fertility data* (asFR or $f_x$) using `get_rosbris()`:
...
```{r, echo=FALSE}
fpall <- dbf5all %>% filter(code == 1100 & territory == "t")
...
data = dbm.1 %>% filter(territory == "t" & sex == "m" & code == 1100)
for(i in 2020:2022){
...
Leslie matrix is a powerful tool for demographic analysis that was introduced by @Leslie. For a nice and detailed introduction to it and matrix projections in general, see @ess_demor, p. 98-122. Also see R package [`demogR`](https://CRAN.R-project.org/package=demogR) and its tutorial [@demogR] for more on "matrix methods" in demography. 
