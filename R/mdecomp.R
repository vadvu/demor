...
#'
#' @return Dataframe with 1st column as overall decomposition (`ex12`), and other columns are decompositions by causes (`ex12_cause(i)`)
#' @export
mdecomp <- function(mx1, mx2, age, sex){
  if (length(mx1)!=length(mx2)){stop("lengths (number of causes) of mx1 and mx2 are not equal")}
  if (length(mx1[[1]])!=length(mx2[[1]]) | length(mx1[[1]])!=length(age)){stop("lengths (number of age-groups) of mx1 and mx2 are not equal")}
  ex12 <- decomp(mx1$all, mx2$all, age = age, sex = sex)$ex12
  mdec <- data.frame(age = age)
  mdec$ex12 <- ex12
  for (i in 2:length(mx1)){
    mdec[paste0(names(mx1)[i])] <- NA
    for(j in 1:nrow(mdec)){
      mdec[j,ncol(mdec)] <- mdec[j,"ex12"]*( mx2[[i]][j]-mx1[[i]][j] ) / ( mx2[[1]][j]-mx1[[1]][j] )
    }
  }
  return(mdec)
}
